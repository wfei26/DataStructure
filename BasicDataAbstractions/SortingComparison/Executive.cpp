 ...
 */
Executive::Executive(int size, const char *order,
        const char *sortType, int threshold) {
    this->size = size;
    ...
    } else if (!strcmp(sortType, "QSIS")) {
        time = sort<double>(quickInsertionSort);

    } else if (!strcmp(sortType, "QSBS")) {
        time = sort<double>(quickBubbleSort);

    } else if (!strcmp(sortType, "MSIS")) {
        time = sort<double>(mergeInsertionSort);

    } else if (!strcmp(sortType, "MSBS")) {
        time = sort<double>(mergeBubbleSort);
 ...
 */
template<class T>
unsigned long Executive::sort(void (sortFunction)(T[], long)) {
 ...
 */
template<class T>
unsigned long Executive::sort(void (sortFunction)(T[], long, int)) {
    unsigned long startTime = getTime();
    sortFunction(array, size, threshold);
    unsigned long endTime = getTime();
    return endTime - startTime;
}
